...
// FIXED VERSION: Identification issues resolved
// - Removed dead shock lambda_CN/xi_lambda
// - Consolidated mu shocks (kept xi_dol, removed eps_mu)
// =====================================================
...
// Monetary policy persistence & Taylor rule
rho_US     = 0.3;
rho_CN     = 0.5;
rho_dol    = 0.5;
rho_mu     = 0.7;
...
chi_tb     = 1.5;
chi_dcp    = 0.4;
lambda_ss  = 0.7;  // Fixed DCP share (not shocked)
...
model(linear);
  ...
  // -------------------------
  // DOLLARIZATION AND RISK PREMIUM (CONSOLIDATED)
  // -------------------------
  // FIX: Removed eps_mu, only xi_dol drives the wedge via dol_CN
  mu     = rho_mu*mu(-1) + gamma_dol*dol_CN;
  dol_CN = rho_dol*dol_CN(-1) + xi_dol;

  // -------------------------
  // CPI INFLATION (linearized)
  // -------------------------
  pi_US_CPI =
    pi_US
  ...
  pi_CN_CPI =
    ...
    + chi_dcp*lambda_ss*de;   // Fixed DCP share (no shock)
  ...
  // -------------------------
  // MEASUREMENT EQUATIONS (×400 scaling)
  // -------------------------
...
shocks;
  ...
  var xi_dol;    stderr 0.1;   // Now the sole driver of mu wedge
end;
...
// -------------------------
// OPTIONAL: CHECK IDENTIFICATION
// -------------------------
// Uncomment to verify parameter identification before estimation:
// identification;