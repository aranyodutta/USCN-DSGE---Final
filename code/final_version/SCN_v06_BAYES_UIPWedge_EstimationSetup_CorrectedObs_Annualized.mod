...
// BAYESIAN ESTIMATION VERSION - CORRECTED
// =====================================================
// FIXES APPLIED:
// 1. Policy rate observables now measure LEVELS (dr = r_US) not changes
// 2. Consistent annualization: growth rates * 400, inflation * 400, rates * 400
// 3. Added prefilter=1 to handle non-zero means in data
// =====================================================
...
model(linear);
  ...
  // -------------------------
  // FIX 1: Policy rates are now LEVELS (not changes)
  // FIX 2: Annualization applied consistently (400 for quarterly to annual %)
  
  // Output growth: annualized quarterly percent change
  dy      = 400*(y_US - y_US(-1)) + e1;
  dy_c    = 400*(y_CN - y_CN(-1)) + e4;
  
  // Inflation: annualized quarterly percent change
  dinfl   = 400*(p_US - p_US(-1)) + e2;
  dinfl_c = 400*(p_CN - p_CN(-1)) + e5;
  
  // Policy rates: annualized levels (not changes)
  dr      = 400*r_US + e3;
  dr_c    = 400*r_CN + e6;
...
// BAYESIAN ESTIMATION
// =====================================================
// FIX 3: Added prefilter=1 to handle non-zero means in data
estimation(
  datafile = data_6,
  first_obs = 1,
  nobs = 93,
